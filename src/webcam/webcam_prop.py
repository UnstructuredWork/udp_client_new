...
class CamProp:
    ...
    def set_resolution(self, value):
        ...
        if value not in self.resolution_list.keys():
            ...
            self.set_width(self.resolution_list[self.get_resolution()][0])
            self.set_height(self.resolution_list[self.get_resolution()][1])
    ...
    def set_format(self, value):
        if value not in ['NV12', 'YUYV', 'H264']:
            value = 'NV12'
            raise Exception(
                f"Pixel format wrong!!! Pixel format of camera must select in ['NV12', 'YUYV', 'H264'] not [{value}]")
